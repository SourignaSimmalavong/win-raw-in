...
def open_window():
    ...
    for device in list_devices():
        label = create_copiable_label(table, text=f'{(device.product_name or device.vendor_name or device.interface_name or device.path)} {device.path}', copy_str=device.path.replace('\\', '\\\\') if device.path else '', bg=tk_rgb(0, 0, 0))
        labels[device] = label
